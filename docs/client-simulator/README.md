...
- Tham số hữu ích: `--segments`, `--manifest=<path>`, `--client=<id>`.
...
4. **Fallback Origin trực tiếp:** Viewer vẫn có thể gọi origin nếu seeder thất bại. Thiết lập `config.streaming.fallback.origin.baseUrl` (mặc định `http://localhost:9000/movies`) và đảm bảo `streamId` đúng định dạng `<movieId>_<quality>` để template `/{movieId}/{quality}/{segmentId}.ts` hoạt động.
