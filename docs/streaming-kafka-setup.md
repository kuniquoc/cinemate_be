...
## 1. Mục tiêu
Tài liệu này hướng dẫn cách chuẩn bị hệ thống Kafka để dịch vụ `streaming-signaling` có thể tiêu thụ sự kiện fan-out cho các phiên xem phim, bao gồm việc đặt tên topic, tạo topic trên từng môi trường và cấu hình ACL/biến môi trường tương ứng. Module `streaming-seeder` không kết nối Kafka nên không cần cấu hình trong tài liệu này.
...
## 5. Cấp quyền ACL
Giả sử dịch vụ sử dụng principal `User:streaming-signaling`.
...
kafka-acls \
  ...
  --command-config admin.properties \
  --add \
  --allow-principal User:streaming-signaling \
  --operation Read \
  --operation Describe \
  --topic stream. \
...
2. Kiểm tra ACL: `kafka-acls --bootstrap-server <broker-list> --list --principal User:streaming-signaling`.
3. Khởi động dịch vụ `streaming-signaling` với các biến môi trường ở trên và quan sát log:
